 ...
 * Documentation: C3C Ethan Schofield showed me how to use the 
 *  Watch section in the debugger. This allowed me to check what
 *  was being stored inside arrays.
 * ===========================================================
 */
...
void RemovePunctuationMakeUpperCase(STRING argWord) {
    ...
    while (*src) {
        if (!isalpha((unsigned char)*src)) {
            ...
            /* Make it uppercase */
            *dst++ = (char)toupper((unsigned char)*src);  // Casted the value to type char
            src++;
        } else if (src == dst) {
...
void PrintWordsToConsole(WordCount* argWordCount) {
    ...
    for (int index = 1; argWordCount[index].word != NULL; index++)  // To start on non-NULL index
    {
...
int NumUniqueWords(WordCount* argWordCount) {
    int i = 0;  // Needs to initialize with a value
    while (argWordCount[i].word != NULL) {
        ++i;  // Needs to increment by 1
    }
...
WordCount* CreateWordCountArray(int argNumberOfWords) {
    ...
    retWordCount =
        malloc((argNumberOfWords + 1) * sizeof(WordCount));  // Not enough memory for WordCount
...
WordCount* ReadStoryFile(STRING argFileName, WordCount* argExclusionList) {
    ...
    if (numWords <= 0) {
        ...
        // open the file
        FILE* filePtr = NULL;  // Needs to be a pointer
...
bool WordExistsInWordCount(WordCount* argWordCount, STRING argBuffer) {
    ...
    return false;  // Needs a false branch to be a Boolean
}
...
void UpdateWordCount(WordCount* argWordCount, STRING argBuffer) {
    ...
    strcpy(argWordCount[i].word, argBuffer);  // Need to use strcpy, not an assignment operator
