...
static int playerType[2] = {HUMAN_PLAYER, AI_PLAYER};
static char *playerTypeNames[2] = {" (Human)", " (AI)"};

static int activePlayer = CUT_PLAYER;
 char *activePlayerName[] = {"Cut's turn", "Short's turn"};  // Modified (removed static) to allow easy display of winner's name
...
int aiPlayer(Graph *graph) {
    ...
    int tries = 0;
    int from = findStartVertex(graph);
    int to = -1;

    for (int i = 0; i < graph->numberVertices; i++) {
        if (graphGetEdge(graph, from, i) == NORMAL_EDGE) {
            to = i;
        }
    }
    if (to == -1) {
        from = findEndVertex(graph);
        for (int i = 0; i < graph->numberVertices; i++) {
            if (graphGetEdge(graph, from, i) == NORMAL_EDGE) {
                to = i;
            }
    ...
    if (to == -1) {
        ...
        for (int i = 0; i < graph->numberVertices; i++) {
            for (int j = 0; j < graph->numberVertices; j++) {
                if (graphGetEdge(graph, i, j) == NORMAL_EDGE) {
                    from = i;
                    to = j;
                }
            }
...
void doTurn(Graph *graph, int mouseX, int mouseY) {
    int turnIsDone = TRUE;
     ...
     if (checkPathCut(graph, findStartVertex(graph), findEndVertex(graph), 0) == 0) {  // Check if cut won
         activePlayerName[0] = "Cut wins!";
         activePlayerName[1] = "Cut wins!";
         printf("Cut wins!");
         turnIsDone = FALSE;
     } else {
         if (checkPathShort(graph, findStartVertex(graph), findEndVertex(graph), 0)) {  // Check if short won
             activePlayerName[0] = "Short wins!";
             activePlayerName[1] = "Short wins!";
             printf("Short wins!");
             turnIsDone = FALSE;
         }
     }
...
int findStartVertex(Graph *graph) {
    for (int i = 0; i < graph->numberVertices; i++) {
        Vertex vertex = ((Vertex*)(graph->vertices))[i];
        if (vertex.type == STARTING_VERTEX) {
            return i;
        }
    }
    return -1;
...
int findEndVertex(Graph *graph) {
    for (int i = 0; i < graph->numberVertices; i++) {
        Vertex vertex = ((Vertex*)(graph->vertices))[i];
        if (vertex.type == ENDING_VERTEX) {
            return i;
        }
    }
    return -1;
...
int checkPathCut(Graph *graph, int start, int end, int depth) {
    if (start == end) {     // Check if end has been reached
        return 1;           // Return 1 if path successful
    }
    if (depth > graph->numberVertices) {    // Check if all possible paths have been traversed
        return 0;                           // Return 0 if path not possible
    }
    printf("checkPathCut: depth: %d, start: %d, end: %d\n", depth, start, end);
    int finished = 0;
    for (int i = 0; i < graph->numberVertices; i++) {       // Check all possible edges connected to start vertex
        if (graphGetEdge(graph, start, i) == LOCKED_EDGE || graphGetEdge(graph, start, i) == NORMAL_EDGE) {
            finished |= (checkPathCut(graph, i, end, depth + 1));
        }
    }
    return finished;   // Catch-all return true
}

int checkPathShort(Graph *graph, int start, int end, int depth) {
    if (start == end) {     // Check if end has been reached
        return 1;           // Return 1 if path successful
    }
    if (depth > graph->numberVertices) {    // Check if all possible paths have been traversed
    ...
    printf("checkPathShort: depth: %d, start: %d, end: %d\n", depth, start, end);
    int finished = 0;
    for (int i = 0; i < graph->numberVertices; i++) {       // Check all possible edges connected to start vertex
        if (graphGetEdge(graph, start, i) == LOCKED_EDGE) {
            finished |= (checkPathShort(graph, i, end, depth + 1));
        }
    }
    return finished;   // Catch-all return true
}

// needs to use playertype to work
int DFSRec(Graph *graph, int *discovered, int visited) {
    discovered[visited] = TRUE;
    for(int i = 0; i < graph->numberVertices; i++) {
        if(graph->edges[visited][i] && !discovered[i]) {
            if(visited == ENDING_VERTEX) {
                return 1;
            } else {
                DFSRec(graph, discovered, visited);
            }
        }
    } 
    return 0;
